...
export class UnavailableBillingAdapter implements BillingAdapter {
  ...
  private toast: { show: (msg: string, type: string) => void } | null

  constructor(toast?: { show: (msg: string, type: string) => void }) {
...
export function getBillingAdapter(toast?: { show: (msg: string, type: string) => void }): BillingAdapter {
  if (!adapterInstance) {
